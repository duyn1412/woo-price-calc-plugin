(function(apiFetch){
    ...
    async function buildSimpleResults(posts, attributes, currentPage, perPage, totalPagesFromAPI = null, totalPostsFromAPI = null){
      ...
      // Add Previous button (left side) - always show
      if (currentPage > 1) {
        html += `<a class="page-numbers prev" href="#" data-page="${currentPage - 1}"><svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none">
  <path d="M16.3337 10.0001H4.66699M4.66699 10.0001L10.5003 15.8334M4.66699 10.0001L10.5003 4.16675" stroke="#ffffff" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round"></path>
</svg> <span>Previous</span></a>`;
      } else {
        html += `<span class="page-numbers prev disabled"><svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none">
  <path d="M16.3337 10.0001H4.66699M4.66699 10.0001L10.5003 15.8334M4.66699 10.0001L10.5003 4.16675" stroke="#ffffff" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round"></path>
</svg> <span>Previous</span></span>`;
      }
      ...
      if (isMobile) {
        ...
        // Add page numbers in range (center)
      ...
      // Add Next button (right side) - always show
      if (currentPage < totalPages) {
        html += `<a class="page-numbers next" href="#" data-page="${currentPage + 1}"><span>Next </span><svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none">
  <path d="M4.66699 10.0001H16.3337M16.3337 10.0001L10.5003 4.16675M16.3337 10.0001L10.5003 15.8334" stroke="#ffffff" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round"></path>
</svg></a>`;
      } else {
        html += `<span class="page-numbers next disabled"><span>Next </span><svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none">
  <path d="M4.66699 10.0001H16.3337M16.3337 10.0001L10.5003 4.16675M16.3337 10.0001L10.5003 15.8334" stroke="#ffffff" stroke-width="1.66667" stroke-linecap="round" stroke-linejoin="round"></path>
</svg></span>`;
      }
    ...
    function addPaginationListeners(targetEl){
      ...
      paginationLinks.forEach(link => {
        link.addEventListener('click', async function(e){
          ...
          // Scroll to top of modules-results-container
          const container = document.querySelector('.modules-results-container');
          if (container) {
    ...
    // Update clear button visibility based on checked checkboxes
    ...
    function initMobileFilters() {
      const triggerBtn = document.querySelector('.trigger-filters-button');
      const form = document.querySelector('form.modules-filters');
      
      if (triggerBtn && form) {
        // Add SVG icon and text to button if not exists
        if (!triggerBtn.querySelector('svg')) {
